...
output_dir_base = 'dynamics/66/output'
single_sim_output_dir = os.path.join(output_dir_base, 'simulation_main')
os.makedirs(single_sim_output_dir, exist_ok=True)

# --- 1. Inicjalizacja ---
fixer_pdb_path = 'dynamics/66/input/5cno_prepared.pdb'
fixed_protein_pdb_path = 'dynamics/66/input/protein_fixed.pdb' 

fixer = PDBFixer(fixer_pdb_path)
...
app.PDBFile.writeFile(fixer.topology, fixer.positions, open(fixed_protein_pdb_path, 'w'))
print(f"Protein structure initially processed by PDBFixer (without addMissingHydrogens) saved to: {fixed_protein_pdb_path}")

# --- 2. Przygotowanie wodorów w białku zgodnie z AMBER ---
print("\nPreparing protein hydrogens using AMBER force field definitions...")
protein_structure_for_H_fix = app.PDBFile(fixed_protein_pdb_path)
...
protein_amber_H_pdb_path = 'dynamics/66/input/protein_amber_H.pdb'
app.PDBFile.writeFile(protein_modeller.topology, protein_modeller.positions, open(protein_amber_H_pdb_path, 'w'))
print(f"  Protein with AMBER-compliant hydrogens saved to: {protein_amber_H_pdb_path}")


protein_topology_final = protein_modeller.getTopology()
protein_positions_final = protein_modeller.getPositions()
# --- KONIEC BLOKU PRZYGOTOWANIA WODORÓW W BIAŁKU ---

# --- 3. Wczytanie struktur ---
# Load ligand using OpenFF Toolkit
ligand_file_path = 'dynamics/66/input/ligand66_h.sdf'
print(f"\nLoading ligand from: {ligand_file_path} for OpenFF parameterization...")
try:
    ligand_off_molecule = Molecule.from_file(ligand_file_path, allow_undefined_stereo=True)
    print("Ligand loaded into OpenFF Molecule object.")
except Exception as e:
    print(f"Error loading ligand {ligand_file_path} with OpenFF: {e}")
...
def run_simulation_logic(sim_output_dir, sim_integrator, sim_topology, sim_positions, sim_system, include_heating, total_steps_production):
    ...
    local_simulation.reporters.append(app.StateDataReporter(stdout, 1000, step=True, progress=True, remainingTime=True, speed=True, totalSteps=total_steps_production + (25000 if include_heating else 0) ))

    if include_heating:
        print('Starting heating phase...')
        initial_temp = 50*unit.kelvin
        final_temp = 300*unit.kelvin
        heating_steps = 2500000  # 25000 * 100
        n_stages = 50 
        steps_per_stage = heating_steps // n_stages
...
# --- 10. Uruchomienie symulacji ---


print("\nRunning Simulation WITH Heating Phase...")
integrator_with_heating = mm.LangevinMiddleIntegrator(temperature, friction, timestep) # New integrator instance
run_simulation_logic(
    sim_output_dir=single_sim_output_dir,
    ...
    total_steps_production=5000000  # 50000 * 100
)

analyze_trajectory_and_plot_histograms(
    xtc_file_path=os.path.join(single_sim_output_dir, 'trajectory.xtc'),
    pdb_topology_file_path=os.path.join(single_sim_output_dir, 'trajectory.pdb'), # Use the PDB from the same run for topology
    output_prefix=os.path.join(single_sim_output_dir, 'analysis')
)


# --- 11. Zakończenie ---
print('\nAll simulations completed!')
print(f"Trajectory for sim with heating saved to: {single_sim_output_dir}")